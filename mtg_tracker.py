...
next to this script (mtg_tracker_data/tracker.db), so your progress is kept
between sessions. Python 3.9+ only, no third-party packages.
"""
...
def _data_dir():
    ...
    env = os.environ.get("MTG_TRACKER_DATA")
    if env:
        return env
    # A "mtg_tracker_data" folder next to the script — or, in the frozen
    # Windows .exe, next to the executable — turns on portable mode.
    roots = [os.path.dirname(os.path.abspath(__file__))]
    if getattr(sys, "frozen", False):
        roots.insert(0, os.path.dirname(os.path.abspath(sys.executable)))
    for root in roots:
        here = os.path.join(root, "mtg_tracker_data")
        if os.path.isfile(os.path.join(here, "tracker.db")):
            return here                               # existing setup wins
    # Renamed MTG Tracker -> Binduno after data already lived under the old
    # folder name for real users — installs from before the rename keep
    # using it rather than silently looking empty; only a fresh install gets
    # the new branded folder.
    if sys.platform == "darwin":
        old = os.path.expanduser("~/Library/Application Support/MTG Tracker")
        if os.path.isfile(os.path.join(old, "tracker.db")):
            return old
        return os.path.expanduser("~/Library/Application Support/Binduno")
    if sys.platform == "win32":
        for cand in ("~/.mtg_tracker", "~/.binduno"):
            p = os.path.expanduser(cand)
            if os.path.isfile(os.path.join(p, "tracker.db")):
                return p
        base = os.environ.get("LOCALAPPDATA") or os.path.expanduser("~")
        return os.path.join(base, "Binduno")
    old = os.path.expanduser("~/.mtg_tracker")
    if os.path.isfile(os.path.join(old, "tracker.db")):
        return old
...
def github_latest(repo):
    """Newest mtg_tracker.py for `repo`: prefer the latest Release, fall back
    to the raw file on the default branch. Returns dict with srcUrl/latest/…"""
    ...
    try:
        ...
        src = next((a["browser_download_url"] for a in rel.get("assets", [])
                    if a.get("name") == "mtg_tracker.py"), None) \
            or f"https://raw.githubusercontent.com/{repo}/{tag}/mtg_tracker.py"
        ...
        src = f"https://raw.githubusercontent.com/{repo}/{br}/mtg_tracker.py"
...
def apply_new_source(c, src, note):
    ...
    if "Binduno" not in src or "def main()" not in src:
        raise ValueError("This does not look like a mtg_tracker.py file.")
    m = re.search(r'VERSION\s*=\s*"([^"]+)"', src)
    newver = m.group(1) if m else "?"
    compile(src, "mtg_tracker.py", "exec")               # syntax check
    target = os.path.abspath(__file__)
    shutil.copy2(target, os.path.join(BASE, "mtg_tracker_previous.py"))
...
en:{
  ...
  "manageApp.desc":"Pick a newer mtg_tracker.py. It is checked for syntax errors, the "+
    "running file is backed up as <code>mtg_tracker_previous.py</code>, then the app "+
    "replaces itself and restarts. No Terminal, no reinstalling.",
  "manageApp.filePickerNote":"Choose mtg_tracker.py — the file picker shows all files "+
  ...
  "manageApp.rebuildDesc":"If the app icon or launcher ever breaks, run "+
    "<code>python3 mtg_tracker.py --install-app</code> once in Terminal.",
...
de:{
  ...
  "manageApp.desc":"Wähle eine neuere mtg_tracker.py. Sie wird auf Syntaxfehler geprüft, "+
    "die laufende Datei als <code>mtg_tracker_previous.py</code> gesichert, dann ersetzt "+
    "sich die App selbst und startet neu. Kein Terminal, keine Neuinstallation.",
  "manageApp.filePickerNote":"mtg_tracker.py auswählen — der Dateiauswahl-Dialog zeigt alle "+
  ...
  "manageApp.rebuildDesc":"Falls Icon oder Starter je kaputtgehen, einmal "+
    "<code>python3 mtg_tracker.py --install-app</code> im Terminal ausführen.",
...
function offline(e){
  ...
  $("#view").innerHTML=dead
   ? `<div class="empty"><h2>No connection to the tracker</h2>
      ...
      <p class="sub">Start it again with <code>python3 mtg_tracker.py</code>
...
def build_windows_exe():
    ...
    Run this ON Windows:  py mtg_tracker.py --build-exe  [--console]
    The .exe needs no Python or anything else on the target machine."""
    if sys.platform != "win32":
        sys.exit("Run this on Windows: py mtg_tracker.py --build-exe")
    try:
...
SCRIPT="$HERE/Resources/mtg_tracker.py"
PY="$HERE/Resources/python/bin/python3"
LOG="$HOME/Library/Logs/Binduno.log"
mkdir -p "$(dirname "$LOG")"

if [ ! -x "$PY" ]; then
  osascript -e 'display alert "Binduno" message "The bundled Python runtime is missing from the app. Re-run the installer (python3 mtg_tracker.py --install-app)." as critical'
  exit 1
fi

if [ ! -f "$SCRIPT" ]; then
  osascript -e 'display alert "Binduno" message "The app is incomplete: mtg_tracker.py is missing from the bundle. Re-run the installer." as critical'
  exit 1
fi
...
def install_app():
    ...
    shutil.copy2(script, os.path.join(res, "mtg_tracker.py"))
...
def main():
    ...
    if srv is None:
        msg = (f"Cannot listen on ports {PORT}-{PORT + 9}: {last_err}\n"
               "Something else is using them. Close it, or set MTG_TRACKER_PORT.")
        print(msg)
        _win_error_box(msg)
        sys.exit(1)
